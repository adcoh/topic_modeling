...
from TopicModeler.base_topic_modeler import BaseTopicModeler
from WCSVNtm.models import SentenceId, WCSVNTConfig
...
class WCSVNtm(BaseTopicModeler):
    def __init__(self, model_config: WCSVNTConfig):
        super().__init__(model_config=model_config)
        self.alpha_word = model_config.alpha_word
        self.alpha_doc = model_config.alpha_doc
    ...
    def statistically_validate_word_pairs(
        ...
        """
        logger.info(
            f"Validating word pairs with {len(word_occurrence)} words, {total_sentences} sentences, alpha={self.alpha_word}"
        )

        try:
            logger.info("Using Polars implementation for vectorized computation")
            return self._statistically_validate_word_pairs_polars(
                word_occurrence, total_sentences
            )
        except ImportError:
            logger.warning(
                "Polars not available, falling back to Python implementation"
            )
            return self._statistically_validate_word_pairs_python(
                word_occurrence, total_sentences
            )
    ...
    def build_validated_word_network(self, edges: list[tuple[str, str]]) -> nx.Graph:
        G = nx.Graph()
    ...
    def build_doc_wordpair_bipartite(
        self,
        tokenized_docs: list[list[list[str]]],
        validated_word_pairs: list[tuple[str, str]],
    ) -> nx.Graph:
        B = nx.Graph()
        for doc_id, sentences in enumerate(tokenized_docs):
    ...
    def statistically_validate_documents(
        self, doc_bipartite: nx.Graph, total_pairs: int
        ...
        """
        try:
            return self._statistically_validate_documents_polars(
                doc_bipartite, total_pairs
            )
        except ImportError:
            return self._statistically_validate_documents_python(
                doc_bipartite, total_pairs
            )
    ...
    def _statistically_validate_documents_polars(
        ...
        doc_neighbors: dict[str, set] = {
            d: set(doc_bipartite.neighbors(d)) for d in docs
        }

        # Check if any document has neighbors (word pairs)
        if not any(doc_neighbors.values()):
            logger.info(
                "No word pairs found for documents, skipping document validation"
            )
            return []
    ...
    def associate_topics_documents(
        self, G_word: nx.Graph, doc_clusters: dict[str, list], fdr_alpha: float = 0.05
    ) -> dict[int, list[str]]:
    ...
    def compute_topic_importance(
        self, G_word: nx.Graph, topics: list[set[str]]
        ...
        for topic_idx, community in enumerate(topics):
            ...
            for word_i in community:
                ...
                for word_j in community:
                    ...
                    edge_exists: int = (
                        1 if word_j in adjacency_dict.get(word_i, {}) else 0
                    )
    ...
    # -- Full Pipeline -----------------------------------------------------------
    def run_topic_modeling(self, documents: list[str]) -> dict[str, t.Any]:
        """Run the complete WCSVNtm pipeline."""
        logger.info("=" * 60)
        logger.info("Starting WCSVNtm Pipeline")
        logger.info("=" * 60)
        logger.info(
            f"Parameters: alpha_word={self.alpha_word}, alpha_doc={self.alpha_doc}"
        )

        # Step 1: Preprocessing
        logger.info("\n--- Step 1: Document Preprocessing ---")
        tokenized = self.preprocess_documents(documents)

        # Step 2: Build bipartite graph
        logger.info("\n--- Step 2: Building Bipartite Graph ---")
        B_sw, word_occ = self.build_sentence_word_bipartite(tokenized)
        total_sent = len(
        ...
        logger.info("\n--- Step 3: Word Pair Validation ---")
        wpairs = self.statistically_validate_word_pairs(word_occ, total_sent)
        logger.info(f"Validated word pairs: {len(wpairs)}")

        # Step 4: Build word network
        logger.info("\n--- Step 4: Building Word Network ---")
        G_word = self.build_validated_word_network(wpairs)
        logger.info(
        ...
        B_dp = self.build_doc_wordpair_bipartite(tokenized, wpairs)
        ...
        logger.info("\n--- Step 8: Community Detection ---")
        topics = self.community_detection(G_word)
        doc_clusters = self.community_detection(G_doc)
        ...
        association = self.associate_topics_documents(
            G_word,
            {
                "topics": topics,
        ...
        logger.info(
            f"Topic-document associations computed for {len(association)} topics"
        )

        # Step 10: Topic importance
        logger.info("\n--- Step 10: Computing Topic Importance ---")
        importance = self.compute_topic_importance(G_word, topics)
        ...
        # TODO: Extend TopicModelResult and return here
        return {
            "word_graph": G_word,
            "doc_graph": G_doc,
            "topics": topics,
...
# TODO: Build incremental version of pipeline
"""
1. Persistence layer for bipartite word and document graphs
2. Incremental update of graphs
3. Persistence layer for word pair dataframes
"""
...
if __name__ == "__main__":
    ...
    model_config = WCSVNTConfig(alpha_word=0.01, alpha_doc=0.01)
    topic_modeler = WCSVNtm(model_config=model_config)
    try:
        results = topic_modeler.run_topic_modeling(docs)
