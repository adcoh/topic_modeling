    ...
    for doc_id, sentences in enumerate(tokenized_docs):
        for sent in sentences:
            ...
            if sentence_id % 1000 == 0:
                logger.info(f"Processed {sentence_id} sentences, {len(word_occurrence)} unique words")

    logger.info(f"Bipartite graph complete: {sentence_id} sentences, {len(word_occurrence)} words, {total_edges} edges")
...
def statistically_validate_word_pairs(
    ...
    """
    logger.info(f"Validating word pairs with {len(word_occurrence)} words, {total_sentences} sentences, alpha={alpha}")
...
def _statistically_validate_word_pairs_polars(
    ...
    interm_result = (
        pairs_df
        .with_columns([
            pl.col("set1").list.set_intersection(pl.col("set2")).list.len().alias("Nij"),
    ...
    logger.info(f"Statistical validation complete: {validated_pairs} pairs validated (p < {alpha_bonf:.6f})")
    
    result = result.select(["word1", "word2"])
    return [row for row in result.iter_rows()]
...
def _statistically_validate_documents_polars(
    ...
    doc_matrix = pl.DataFrame({
        "doc": docs,
        "neighbor_sets": [doc_neighbors[d] for d in docs]
    }).lazy()
    
    # Generate all document pairs efficiently
    pairs_df = pl.DataFrame({
        "doc1": [docs[i] for i in range(len(docs)) for j in range(i + 1, len(docs))],
        "doc2": [docs[j] for i in range(len(docs)) for j in range(i + 1, len(docs))]
    }).lazy()
    ...
    interm_result = (
        ...
        .with_columns([
            ...
            pl.col("set1").list.set_intersection(pl.col("set2")).list.len().alias("Vij")
        ])
        .filter(pl.col("Vij") > 0)
    ).collect()
    ...
    T = len(interm_result)
    alpha_bonf = alpha / T
    logger.info(f"Document validation Bonferroni correction: T={T} tests, alpha_bonf={alpha_bonf:.6f}")
...
def associate_topics_documents(
    ...
    for topic_id, topic_words in enumerate(doc_clusters["topics"]):
        for doc in doc_clusters["docs"]:
            ...
            tests.append((topic_id, doc, pval))
...
def compute_topic_importance(
    G_word: nx.Graph, topics: list[set]
) -> dict[int, dict[str, float]]:
    importance: dict[int, dict[str, float]] = {}
    E = G_word.size()
    deg = dict(G_word.degree())
    A = nx.to_dict_of_dicts(G_word)
    for tidx, community in enumerate(topics):
        ...
        for wi in community:
            ...
            for wj in community:
                if wi == wj:
                    continue
                Aij = 1 if wj in A[wi] else 0
                M_wi += Aij - deg[wi] * deg[wj] / (2 * E)
            M_wi /= 2 * E
            contribs[wi] = M_wi
            denom += M_wi
        importance[tidx] = {wi: (M / denom) for wi, M in contribs.items()}
    return importance
...
def run_wcsvntm(
    ...
    logger.info(f"Word network: {G_word.number_of_nodes()} nodes, {G_word.number_of_edges()} edges")
    ...
    logger.info(f"Document-word pair bipartite: {len(documents)} docs, {total_pairs} word pairs")
    ...
    logger.info(f"Document network: {G_doc.number_of_nodes()} nodes, {G_doc.number_of_edges()} edges")
...
if __name__ == "__main__":
    ...
    try:
        ...
        if results["importance"]:
            logger.info(f"Topic importance computed for {len(results['importance'])} topics")
